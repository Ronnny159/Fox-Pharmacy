...
CREATE OR REPLACE PACKAGE PKG_PHARMASMART_FEFO AS
    ...
    FUNCTION OBTENER_LOTES_POR_PRODUCTO(p_producto_id NUMBER) RETURN T_LOTE_TABLA PIPELINED;
    
    -- Obtener todos los lotes activos
    FUNCTION OBTENER_LOTES_ACTIVOS RETURN T_LOTE_TABLA PIPELINED;
...
CREATE OR REPLACE PACKAGE BODY PKG_PHARMASMART_FEFO AS
    ...
    FUNCTION OBTENER_LOTES_POR_PRODUCTO(p_producto_id NUMBER) RETURN T_LOTE_TABLA PIPELINED IS
    BEGIN
        FOR rec IN (
            ...
            PIPE ROW (T_LOTE_INFO(rec.ID, rec.CODIGO_LOTE, rec.PRODUCTO_ID,
                                  rec.FECHA_VENCIMIENTO, rec.PRECIO_COMPRA,
                                  rec.PRECIO_VENTA, rec.CANTIDAD_ACTUAL, rec.ESTADO));
        END LOOP;
    END;

    FUNCTION OBTENER_LOTES_ACTIVOS RETURN T_LOTE_TABLA PIPELINED IS
    BEGIN
        FOR rec IN (
            SELECT ID, CODIGO_LOTE, PRODUCTO_ID, FECHA_VENCIMIENTO,
                   PRECIO_COMPRA, PRECIO_VENTA, CANTIDAD_ACTUAL, ESTADO
